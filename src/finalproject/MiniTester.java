  ...
  try {
   ...
   for (String v : vertices) {
    if (searchEngine.internet.getPageRank(v) <= 0) {
     comment = comment + " Page rank for vertex " + v + " is negative. Page rank should be positive at each iteration." + "\n";
     result = false;
    }
   }
...
//                long durationFast = TimeUnit.NANOSECONDS.toMicros(endTime-startTime);
                ...
                System.out.println("Speed quotient: " + ((double) durationSlow) / durationFast);
 ...
 public static void main(String[] args) {
  ...
  g.write(g.gradeString(total, 50, "\nCalculating total for all minitester tests:\n"));
